...
Die Feldzuordnung steuert für Projekte und Jobs Titel, Untertitel, Beschreibung, Veranstalter/Kunde, Veranstaltungs- beziehungsweise Jobtyp, Datum, Einlass, Beginn, Pause, Ende, Spielort, Raum, öffentliche Hinweise, Ticket-Link und Ticketstatus. Mit `__project.<Feld>` kann ein Job gezielt einen Wert seines übergeordneten Projekts übernehmen; `__none` deaktiviert die Übernahme eines optionalen Feldes. Detailantworten werden sowohl direkt als auch in den easyjob-üblichen Hüllen `Data`, `Project`, `Job` oder `Item` verarbeitet.

Der Verbindungstest ruft zuerst den laut Hersteller ohne Anmeldung verfügbaren Endpunkt `Common/GetGlobalWebSettings` auf und prüft danach getrennt Token/Anmeldung, Projektliste, Jobliste, Details und Raumkalender. Dadurch lässt sich ein Netzwerk- oder WebApi-Problem von falschen Zugangsdaten und fehlenden Leserechten unterscheiden. Die Antwort enthält die erkannten WebApi-Einstellungen sowie stufenweise Diagnosen.

Die Importvorschau liest nicht nur `Projects/List`, sondern für jedes angezeigte Projekt auch `Projects/Details/?id=…`, die enthaltenen `Jobs/Details/?id=…`, `Jobs/List` für Jobtyp und Status sowie `Rooms/Calendar?showroomuse=true`. Zeitraumparameter werden im von easyjob dokumentierten Format `JJJJ-MM-TT` übertragen. Die Antwort enthält eine Projekt→Jobs-Hierarchie, Qualitätsfehler und -warnungen, normalisierte Raumnutzungen und unter `fieldAnalysis` alle erkannten Feldpfade, Datentypen, Häufigkeiten und bis zu drei gekürzte Beispielwerte. Importiert werden können nur Projekte, nur Jobs oder beide Objektarten. Raumnutzungen werden beim Import automatisch als Veranstaltungsablauf übernommen.

Im Raumkalender ist die `Id` eines `IdT=76`-Datensatzes eine Kalender-/Zuordnungs-ID und keine Job-ID. Kiosky extrahiert deshalb die Jobnummer aus der dokumentierten Beschriftung, gleicht sie gegen `Jobs/List` beziehungsweise `Jobs/Details` ab und übernimmt nur eindeutig zugeordnete `IdT=77`-Raumnutzungen. Referenzzeilen mit `IsRef=true` werden als Dubletten ignoriert; nicht eindeutige Zuordnungen erscheinen als Warnung und werden nicht stillschweigend einem fremden Job zugewiesen.

Die von EasyJob dokumentierten lokalen Zeitwerte ohne Offset werden ausdrücklich als `Europe/Berlin` interpretiert und kanonisch in UTC gespeichert. Die Vorschau kennzeichnet diese Annahme; ungültige Zeitwerte oder ein Ende vor dem Start blockieren den jeweiligen Datensatz.
